...
PARAMETERS:
    -u, --url <URL>         URL van de YouTube video (verplicht)
    -b, --base-dir <DIR>    Basis directory (default: $HOME/videos)
    -o, --output-dir <DIR>  Eigen output directory
...
# ============================================
if [ -z "$URL" ]; then
    echo "${C_RED}[ERROR] -u/--url is verplicht${C_RESET}" >&2
    usage
    exit 1
fi
if [[ ! "$URL" =~ ^https?:// ]]; then
    echo "${C_RED}[ERROR] Ongeldige URL. Moet beginnen met http:// of https://${C_RESET}" >&2
    exit 1
fi
...
META="$(yt-dlp ${JS_ARGS[@]+"${JS_ARGS[@]}"} --dump-json "$URL")"
if [ -z "$META" ]; then
